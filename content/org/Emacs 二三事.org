...
** Thoughts
- <2024-10-18 Fri> Restart Emacs when changing displayer.
...
** Let
The =let= creates a name for a /local variable/ that overshadows any use of the same name outside the =let= expression (in computer science jargon, we call this /binding/ the variable).[fn:3]
...
#+begin_src dot :file graph.pdf :cmdline -Kdot -Tpdf
  digraph G {
    A -> B;
    B -> C;
    C -> A;
...
<2026-01-13 Tue>现在还没用上期待的 preview，不过 =org-xlatex= 已经很好用了。
...
试了一下，完全看不了我想看的书（textbook full of formulas）. 感觉完全不适合看公式和图多的书和论文，看看别的可能还行?[fn:4]
...
<2026-01-13 Tue>目前完全作为笔记功能使用，阅读文献还是用 zotero 为好.
...
配置方式可以参考[[https://rime.im/docs/][帮助文档]], 我直接使用了雾凇拼音的[[https://github.com/Mark24Code/rime-auto-deploy][安装脚本]].
...
*** Punctuator
全部使用英文标点, 需要直接更改对应输入方案的 yaml 文件.[fn:2]
...
Good reference: [[https://karthinks.com/software/fifteen-ways-to-use-embark/][Fifteen ways to use embark]].
...
* Footnotes
[fn:4]不是很实用, 还是集中在对 org 做笔记和整理笔记上比较好. 

[fn:3]When using =setq= in =let=, it's a locally modification.

[fn:2]rime-ice.schema.yaml in .emacs.d/rime works for me. 
